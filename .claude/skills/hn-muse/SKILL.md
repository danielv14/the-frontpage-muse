...
2. **Create a feature branch:**
   ...
   ```
   Use today's date. If the branch already exists (e.g. re-running on the same day), use `post/YYYY-MM-DD-v2`, `post/YYYY-MM-DD-v3`, etc.
...
5. **Create a pull request:**
   Use `gh pr create` with a descriptive title and body. The PR body should include:
   - A brief note on the creative direction taken
   - The list of HN stories that inspired the post
   - A short excerpt or teaser from the content

   Example:
   ```
   gh pr create --title "New post: <creative title>" --body "$(cat <<'EOF'
   ## Today's Muse

   <Brief description of creative direction>
   ...
   ## Excerpt
   > A tantalizing quote from the post...

   ---
   Generated by The Frontpage Muse
   EOF
   )"
   ```

Done. One invocation, one PR. The post is ready for review and merge.
