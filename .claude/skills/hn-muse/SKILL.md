...
- **Format freshness (hard rule):** Use Glob on `src/content/posts/*.md` and read the frontmatter `format` field of the **8 most recent** posts. If the pitch's proposed format appears in that list, redirect. No exceptions, and no special cases per format — the rule is the same whether the recent run is essays, satires, stories, or anything else. The corpus has historically skewed toward `essay` and `satire`, but the defense is general: if it's been used in the last 8 days, it's off the menu.
