...
4. **An idle notification is stamped when the writer went idle, not when it reaches you. Compare timestamps before nudging.** The writer can only wait for the editor's reply by ending its turn, so every pitch legitimately produces an idle notification at pitch time; its delivery is delayed and can land AFTER the approval has been sent, reading as "got the approval, then stalled". If the notification's embedded timestamp predates the approval message, it is stale: the writer is most likely already writing (composing a draft takes minutes, so an empty disk moments after approval is not evidence of a stall). Nudge only when the idle postdates the approval, or when several minutes pass with no draft-ready message and no file on disk. (Observed 2026-07-31: a stale pitch-time idle triggered a spurious nudge that crossed mid-air with the draft-ready message; the writer's re-read-before-reacting discipline absorbed the collision.)
